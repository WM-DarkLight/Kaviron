...
interface EnterpriseChoiceButtonsProps {
  choices: Choice[]
  onChoiceSelected: (choice: Choice) => void
  disabled?: boolean
}

export function EnterpriseChoiceButtons({ choices, onChoiceSelected, disabled = false }: EnterpriseChoiceButtonsProps) {
      ...
      {choices.map((choice, index) => (
        <button
          ...
          className={cn(
            ...
            focusedIndex === index && "bg-blue-800 ring-2 ring-blue-400",
            disabled && "opacity-50 cursor-not-allowed",
          )}
          aria-pressed={focusedIndex === index}
        >
          <div className="flex items-center">
            <div className="mr-2 w-2 h-2 rounded-full bg-blue-400"></div>
